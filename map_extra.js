...
frameName.innerHTML = "Canada";
var listdoc=document.querySelectorAll(".sozi-title-level-0"), Treaties=[],ListVisible=false;
...
if(Treaties[i].name.length>cutText.length){cutText+="..."}
...
function toggleList(condt){
	if(condt){ListVisible=false} else {ListVisible=!ListVisible}
	if(ListVisible){
...
soziPresentationData.frames.some(function(el, i) {
	...
	frameName.textContent = thisTitle;
